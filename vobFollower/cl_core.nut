...
{
    posOffset = { x = 0, y=0, z=0},//x lewo prawo ,y gora d�, z prz�d ty�
    angleOffset = {x = 0 , y = 0, z = 0}
}
offsetTable["NC_LOB_LOG1.3DS"] <- 
{
    posOffset = { x = 0, y=0, z=30},
    angleOffset = {x = 0 , y = 0, z = 0} // yaw wok� osi gracz
}
...
function RemoveVobWithVisualFor(pid,visual)
{
    foreach(index,table in vobTable)
    {
        if(table.pid == pid && table.vob.visual == visual)
        {
            ...
            table.vob = null
            delete vobTable[index] //mog� by� dwie kopie, na razie to nie ma sensu bo ten sam offset ale mo�e w przysz�o��i tego breaka mozna wyjebac
